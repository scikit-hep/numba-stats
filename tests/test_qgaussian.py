...
@pytest.mark.parametrize("q", (1.0, 1.5))
def test_integrate(q):
    par = q, 1, 2
    for lo, hi in ((-3.0, 4.0), (0.5, 0.7), (4.0, -3.0), (-100.0, 100.0), (2.5, 6.0)):
        got = qgaussian.integrate(lo, hi, *par)
        expected = np.diff(qgaussian.cdf([lo, hi], *par))[0]
        assert_allclose(got, expected, rtol=1e-9, atol=1e-15)
    assert_allclose(qgaussian.integrate(-np.inf, np.inf, *par), 1)
...
def test_integrate_njit():
    @nb.njit
    def test(lo, hi):
        return qgaussian.integrate(lo, hi, 1.5, 1.0, 2.0)

    expected = qgaussian.integrate(-1.0, 1.5, 1.5, 1.0, 2.0)
